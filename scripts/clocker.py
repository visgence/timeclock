...
class Clocker:
    '''
    Clocker is a simple utility class for verifing the user names of employees, checking to see if they can clock
    in/out, and actually clocking them in/out.
    '''
    ...
    def can_clock(self, user_name, status, connection):
        ...
        try:
            ...
            if(result[0] == None and (status == 'In' or status == 'in')):
                return True
            ...
            if((status == 'In' or status == 'in') and result[2] != None):
                return True
